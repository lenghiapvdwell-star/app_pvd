...
def sync_and_save():
    # Đồng bộ dữ liệu từ bảng editor vào session_state trước khi lưu
    if "main_editor" in st.session_state and "edited_rows" in st.session_state.main_editor:
        # Streamlit tự động quản lý qua key, chúng ta chỉ cần ra lệnh lưu
        pass
    try:
...
# --- 4. GIAO DIỆN TIÊU ĐỀ (VIẾT HOA TOÀN BỘ) ---
...
with tabs[0]: 
    ...
    # GIẢI PHÁP TRIỆT ĐỂ: Sử dụng Key để Streamlit tự quản lý State
    # Không gán ngược trực tiếp kết quả hàm vào session_state.db
    st.data_editor(
        st.session_state.db,
    ...
    # Dữ liệu từ bảng sẽ tự động đồng bộ vào session_state.db qua cơ chế nội bộ của Key
