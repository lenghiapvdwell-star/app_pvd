...
with tabs[0]: 
            ...
            with st.form("input_form"):
                ...
                if st.form_submit_button("✅ XÁC NHẬN NHẬP DỮ LIỆU", use_container_width=True):
                    if isinstance(dates, tuple) and len(dates) == 2 and sel_staff:
                        ...
                        # QUAN TRỌNG: st.rerun giúp bảng editor nhận dữ liệu mới mà không bị lỗi xung đột
                        st.rerun()
    ...
    # KHẮC PHỤC TRIỆT ĐỂ: Dùng bản copy để hiển thị
    # Streamlit sẽ không báo lỗi Type Compatibility vì nó không cố ghi đè trực tiếp vào object gốc của session_state khi đang render
    input_df = st.session_state.db.copy()
    ...
    edited_df = st.data_editor(
        ...
        height=600,
        num_rows="dynamic",
        key="main_table_v3" # Thay đổi key để reset cache nếu cần
    )
    
    # Cập nhật ngược lại session_state khi có thay đổi trên bảng
...
with tabs[1]: # GIÀN KHOAN
    ...
    with cg1:
        ...
        edited_g = st.data_editor(g_df, num_rows="dynamic", use_container_width=True, key="rig_editor")
    with cg2:
...
with tabs[2]: # NHÂN VIÊN
    ...
    with cs1:
        ...
        edited_s = st.data_editor(st.session_state.db[s_cols], num_rows="dynamic", use_container_width=True, key="staff_editor")
    with cs2:
