...
def apply_logic(df, curr_m, curr_y, rigs):
    ...
    for idx, row in df_calc.iterrows():
        # Check either English or Vietnamese headers
        name_val = row.get('Full Name', row.get('Họ và Tên', ''))
        if not str(name_val).strip(): continue
        ...
        # Balance columns
        old_bal_key = 'Previous Bal' if 'Previous Bal' in df_calc.columns else 'Tồn cũ'
        new_bal_key = 'Total CA' if 'Total CA' in df_calc.columns else 'Tổng CA'
        
        ton_cu = pd.to_numeric(row.get(old_bal_key, 0), errors='coerce')
        df_calc.at[idx, new_bal_key] = round(float(ton_cu if not pd.isna(ton_cu) else 0.0) + accrued, 1)
    return df_calc

def push_balances_to_future(start_date, start_df, rigs):
    current_df = start_df.copy()
    current_date = start_date
    name_key = 'Full Name' if 'Full Name' in current_df.columns else 'Họ và Tên'
    total_key = 'Total CA' if 'Total CA' in current_df.columns else 'Tổng CA'
    prev_key = 'Previous Bal' if 'Previous Bal' in current_df.columns else 'Tồn cũ'
    ...
    for i in range(1, 13 - current_date.month):
        ...
        try:
            ...
            balances = current_df.set_index(name_key)[total_key].to_dict()
            for idx, row in next_df.iterrows():
                name = row.get(name_key, row.get('Họ và Tên'))
                if name in balances: 
                    # Set the 'Previous Bal' of the next month
                    next_target_key = 'Previous Bal' if 'Previous Bal' in next_df.columns else 'Tồn cũ'
                    next_df.at[idx, next_target_key] = balances[name]
    ...
    with st.spinner(f"Synchronizing data..."):
        ...
        if df_raw.empty:
            ...
            if not prev_df.empty:
                prev_name_key = 'Full Name' if 'Full Name' in prev_df.columns else 'Họ và Tên'
                prev_total_key = 'Total CA' if 'Total CA' in prev_df.columns else 'Tổng CA'
                balances = prev_df.set_index(prev_name_key)[prev_total_key].to_dict()
                for idx, row in df_raw.iterrows():
                    if row['Full Name'] in balances: df_raw.at[idx, 'Previous Bal'] = balances[row['Full Name']]
        else:
            # Rename headers from VN to EN if they exist in the Sheet
            rename_map = {'STT': 'No.', 'Họ và Tên': 'Full Name', 'Công ty': 'Company', 'Chức danh': 'Title', 'Tồn cũ': 'Previous Bal', 'Tổng CA': 'Total CA'}
        ...
        if sheet_name == now.strftime("%m_%Y"):
            ...
            if has_updated:
                ...
                st.toast(f"⚡ Auto-filling data until day {target_day:02d}!", icon="✅")
...
with t1:
    ...
    if c1.button("📤 SAVE & UPDATE YEARLY", type="primary", use_container_width=True):
        with st.spinner("Saving and pushing balances to future months..."):
    ...
    with st.expander("🛠️ QUICK INPUT TOOL"):
        ...
        # Internal mapping to keep logic codes consistent
        status_map = {"Offshore": "RIG", "CA": "CA", "Workshop": "WS", "Holiday": "LỄ", "Annual Leave": "NP", "Sick Leave": "ỐM", "Clear": "XÓA"}
        
        rig = r2.selectbox("Rig Name:", st.session_state.GIANS) if stt == "Offshore" else status_map[stt]
...
# --- BIỂU ĐỒ / CHARTS ---
with t2:
    st.subheader(f"📊 Personnel Statistics for {curr_y}")
    ...
    if sel_name:
        ...
        with st.spinner("Aggregating yearly data..."):
            for m in range(1, 13):
                ...
                if not m_df.empty and sel_name in m_df[name_col].values:
                    ...
                    for k, v in counts.items():
                        if v > 0: yearly_data.append({"Month": f"Month {m}", "Category": k, "Days": v})
        
        if yearly_data:
            df_chart = pd.DataFrame(yearly_data)
            fig = px.bar(df_chart, x="Month", y="Days", color="Category", barmode="stack", text="Days", template="plotly_dark",
                         color_discrete_map={
            ...
            pv = df_chart.pivot_table(index='Category', columns='Month', values='Days', aggfunc='sum', fill_value=0).astype(int)
            pv['YEAR TOTAL'] = pv.sum(axis=1)
            st.table(pv)
