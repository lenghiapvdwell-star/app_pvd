...
if 'db' not in st.session_state:
    with st.spinner(f"🚀 Đang tải dữ liệu {sheet_name}..."):
        ...
        try:
            ...
            init_data = {'STT': range(1, len(NAMES_66) + 1), 'Họ và Tên': NAMES_66, 'Công ty': 'PVDWS', 'Chức danh': 'Casing crew', 'Job Detail': '', 'CA Tháng Trước': [float(b_map.get(n, 0.0)) for n in NAMES_66], 'Quỹ CA Tổng': 0.0}
        ...
        if has_updates: 
            ...
            st.toast("🤖 Đã tự động Autofill ngày mới!", icon="✅")
...
with t1:
    ...
    if bc1.button("📤 LƯU CLOUD", type="primary", use_container_width=True):
    ...
    if bc2.button("🔄 LÀM MỚI", use_container_width=True):
        st.cache_data.clear(); del st.session_state.db; st.rerun()
    
    buf = io.BytesIO()
    st.session_state.db.to_excel(buf, index=False)
    bc3.download_button("📥 XUẤT EXCEL", buf.getvalue(), f"PVD_{sheet_name}.xlsx", use_container_width=True)
    ...
    with st.expander("🛠️ CÔNG CỤ CẬP NHẬT NHANH"):
        ...
        f_status = r1.selectbox("Trạng thái:", ["Xóa trắng", "Đi Biển", "CA", "WS", "NP"])
            ...
            for person in f_staff:
                ...
                for i in range((f_date[1] - f_date[0]).days + 1):
                    ...
                    if d.month == curr_month:
                        cols = [c for c in DATE_COLS if c.startswith(f"{d.day:02d}/")]
                        if cols: st.session_state.db.at[idx, cols[0]] = "" if f_status == "Xóa trắng" else f_val
    ...
    ed_df = st.data_editor(
        st.session_state.db[cols_to_show], use_container_width=True, height=600, hide_index=True,
        column_config={
...
with t2:
    st.subheader(f"📊 Thống kê nhân sự {curr_year}")
    sel_name = st.selectbox("🔍 Nhân viên:", NAMES_66)
    # (Phần Plotly giữ nguyên như code của bạn)
