...
# Tối ưu CSS để giao diện nhẹ và nhanh hơn
st.markdown("""
    <style>
    ...
    /* Giảm hiệu ứng chuyển cảnh để tạo cảm giác phản hồi tức thì */
...
def clean_dataframe(df):
    ...
    df = df.astype(object)
    df = df.fillna("")
    # Xử lý nhanh hơn bằng vectorization thay vì apply lambda từng ô
    return df.replace(["nan", "NaN", "None", "nat", "None", "<na>"], "")

@st.cache_data(ttl=600) # Giữ dữ liệu trong cache 10 phút để quay lại tháng cũ là hiện ngay
...
# Khởi tạo hoặc nạp dữ liệu vào Session State (Chỉ nạp khi đổi tháng)
if 'active_sheet' not in st.session_state or st.session_state.active_sheet != sheet_name:
    ...
    if not df_curr.empty:
        # Áp dụng tồn cũ ngay khi load
        if 'Họ và Tên' in df_curr.columns:
            df_curr['CA Tháng Trước'] = df_curr['Họ và Tên'].map(balance_map).fillna(0.0)
...
# Bổ sung cột ngày thiếu
for col in DATE_COLS:
    if col not in st.session_state.db.columns: st.session_state.db[col] = ""

# --- 6. HÀM TÍNH TOÁN (TỐI ƯU TỐC ĐỘ) ---
...
with t1:
    # VÙNG FRAGMENT: Chỉ vùng này chạy lại khi sửa bảng hoặc bấm nút, cực kỳ mượt!
    @st.fragment
    def render_data_section():
        ...
        # BẢNG DỮ LIỆU SẮP XẾP CỘT CẠNH NHAU
