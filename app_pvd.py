...
# --- 2. DANH SÁCH 66 NHÂN SỰ CỐ ĐỊNH (PHÒNG HỜ KHI SHEET LỖI) ---
...
# --- 3. KẾT NỐI & LOAD DATA (FIX LỖI INDEXERROR) ---
...
def load_all_settings():
    ...
    # 2. Load Nhân sự (Kiểm tra kỹ lỗi Index)
    names = []
    for s_name in ["nhansu", "999s"]:
        ...
        if not df_n.empty:
            ...
            elif len(df_n.columns) >= 2: # Nếu không có tên cột đúng, lấy cột thứ 2
                names = [str(n).strip() for n in df_n.iloc[:, 1].dropna().tolist() if str(n).strip()]
            elif len(df_n.columns) == 1: # Nếu chỉ có 1 cột
                names = [str(n).strip() for n in df_n.iloc[:, 0].dropna().tolist() if str(n).strip()]
            
            if names: break # Nếu đã lấy được tên thì dừng vòng lặp
    
    # Nếu tất cả các sheet đều trống, dùng list 66 người mặc định
...
if df_main.empty:
    ...
    # Tự động lấy tồn cũ tháng trước
...
# --- 6. AUTO-FILL (XUYÊN THÁNG) ---
now = datetime.now()
if sheet_now == now.strftime("%m_%Y") and now.hour >= 6:
    updated = False
    # Ngày 01 lấy từ cuối tháng trước
    if now.day == 1:
...
with tab1:
    ...
    with st.expander("🛠️ CÔNG CỤ CẬP NHẬT NHANH"):
        ...
        sel_names = c_n.multiselect("Nhân viên:", st.session_state.NAMES)
        date_range = c_d.date_input("Từ ngày - Đến ngày:", value=(date(yy, mm, 1), date(yy, mm, 1)))
        
        r1, r2 = st.columns(2)
        mode = r1.selectbox("Loại hình:", ["Đi Biển", "CA", "WS", "NP/Ốm", "Xóa"])
        val_fill = ""
        if mode == "Đi Biển": val_fill = r2.selectbox("Chọn Giàn:", st.session_state.GIANS)
        elif mode == "CA": val_fill = "CA"
        elif mode == "WS": val_fill = "WS"
        elif mode == "NP/Ốm": val_fill = "NP"
                    ...
                    if curr.month == mm:
                        ...
                        if col_m:
                            for name in sel_names:
                                ...
                                if idxs: current_df.at[idxs[0], col_m[0]] = val_fill
                    curr += timedelta(days=1)
                st.success("Đã áp dụng, hãy nhấn 'LƯU'!")

    # Nút bấm
    b1, b2, b3 = st.columns([2, 2, 4])
    if b1.button("📤 LƯU DỮ LIỆU", type="primary"):
...
with tab2:
    st.plotly_chart(px.bar(current_df, x='Họ và Tên', y='Tổng CA', title="Tồn CA tháng hiện tại"), use_container_width=True)
