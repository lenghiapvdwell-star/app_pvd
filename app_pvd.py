import streamlit as st
import pandas as pd
from streamlit_gsheets import GSheetsConnection

# 1. Cấu hình trang
st.set_page_config(page_title="PVD 2026", layout="wide")

# 2. Thông tin tài khoản (Dán cứng để tránh lỗi Secrets)
creds = {
    "project_id": "pvd-management-87",
    "private_key_id": "ef315ddc41c52034e6f3897694a8af63de3c0fdd",
    "private_key": "-----BEGIN PRIVATE KEY-----\\\\IA5dlFVv3qQUTFtPG9REUuc=\n-----END PRIVATE KEY-----\n",
    "client_email": "pvd-sync@pvd-management-87.iam.gserviceaccount.com",
}

# 3. Kết nối
try:
    # Làm sạch key
    creds["private_key"] = creds["private_key"].replace("\\n", "\n")
    
    # Lấy link spreadsheet từ Secrets
    url = st.secrets["connections"]["gsheets"]["spreadsheet"]
    
    # Kết nối
    conn = st.connection("gsheets", type=GSheetsConnection, **creds, spreadsheet=url)
    
    st.title("PVD PERSONNEL 2026")
    df = conn.read(worksheet="PVD_Data", ttl=0)
    st.success("✅ KẾT NỐI THÀNH CÔNG!")
    st.dataframe(df)

except Exception as e:
    st.error(f"❌ LỖI: {e}")
