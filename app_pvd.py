...
# 3. CSS TINH GỌN (Chữ nhỏ lại, tối ưu không gian)
st.markdown("""
    <style>
    .stApp { background-color: #0E1117; color: #FFFFFF; }
    html, body, [class*="css"] { font-size: 14px !important; } /* Chữ nhỏ lại */
    .main-title-text { font-size: 32px !important; font-weight: 800; color: #3b82f6; text-align: center; margin: 0; }
    .stButton>button { height: 2.5em; font-size: 14px !important; }
...
# JS Kéo chuột trái để cuộn ngang
components.html("""
<script>
...
# 6. QUÉT SỐ DƯ (GÓC TRÁI - LOGI CHUẨN)
st.markdown("---")
...
# 7. BẢNG TỔNG HỢP (Khung nhỏ, kéo cuộn, màu sắc)
...
st.session_state.db = st.data_editor(
    ...
    use_container_width=True, height=600, # Khung bảng nhỏ lại
    column_config=col_cfg,
    disabled=['STT', 'Nghỉ Ca Còn Lại']
)

# 8. XUẤT FILE
st.download_button("📥 EXCEL", data=BytesIO().getvalue(), file_name="PVD_2026.xlsx")
