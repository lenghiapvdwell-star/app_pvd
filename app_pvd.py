...
# --- 6. HÀM TÍNH TOÁN (KHÔNG SYNC) ---
def recalculate_ca(df):
    hols = [date(2026,1,1), date(2026,4,30), date(2026,5,1), date(2026,9,2),
            date(2026,2,16), date(2026,2,17), date(2026,2,18), date(2026,2,19)]
    df_calc = df.copy().fillna("").replace(["nan", "NaN"], "")
...
# --- 7. AUTOFILL 8H SÁNG (CÓ TRY-EXCEPT) ---
now = datetime.now()
today_str = now.strftime("%Y-%m-%d")
if "last_autofill_date" not in st.session_state: st.session_state.last_autofill_date = None

if st.session_state.last_autofill_date != today_str and now.hour >= 8:
    df_auto = st.session_state.db.copy().fillna("").replace(["nan", "NaN"], "")
    today_day = now.day
    updated = False
        ...
        for col in DATE_COLS:
            ...
            if curr_val in ["", "NAN", "NONE"]:
                if d_int <= today_day and last_val:
                    ...
                    updated = True
            else: last_val = curr_val
    if updated:
        try:
            st.session_state.db = recalculate_ca(df_auto)
            conn.update(worksheet=sheet_name, data=st.session_state.db)
            st.session_state.last_autofill_date = today_str
            st.toast("Auto-fill thành công!", icon="✅")
        except: st.warning("⚠️ Lỗi API Google: Không thể tự động lưu Autofill.")
    ...
    with bc1:
        if st.button("📤 LƯU CLOUD (DÙNG KHI XONG)", type="primary", use_container_width=True):
            try:
                ...
                st.error("❌ Lỗi API: Google Sheets đang quá tải. Hãy đợi 1 phút rồi thử lại.")
...
with t2:
    st.info("Biểu đồ năm hiển thị dữ liệu đã lưu trên Cloud.")
