...
# Tăng tốc độ load bằng cách ẩn menu mặc định không cần thiết
st.markdown("""
    <style>
    ...
    /* Giảm hiệu ứng nhấp nháy khi load */
    .stDataFrame {transition: opacity 0.3s ease-in-out;}
    </style>
    """, unsafe_allow_html=True)

# --- 2. KẾT NỐI & CACHING (Nâng cấp quan trọng) ---
conn = st.connection("gsheets", type=GSheetsConnection)

@st.cache_data(ttl=600) # Lưu bộ nhớ đệm 10 phút để chuyển tháng cực nhanh
def fetch_data(s_name):
    try:
...
def save_to_cloud(worksheet_name, df):
    ...
    try:
        ...
        st.cache_data.clear() # Xóa cache để lần sau load sẽ lấy dữ liệu mới vừa lưu
        return True
    except Exception as e:
        st.error(f"Lỗi kết nối: {e}")
        return False

# --- 3. LOGIC CỐ ĐỊNH (GIỮ NGUYÊN) ---
...
# --- 5. TẢI DỮ LIỆU (Tối ưu chống trắng bảng) ---
if 'active_sheet' not in st.session_state or st.session_state.active_sheet != sheet_name:
    st.session_state.active_sheet = sheet_name
    if 'db' in st.session_state: del st.session_state.db

if 'db' not in st.session_state:
    df_load = fetch_data(sheet_name)
    if not df_load.empty:
        ...
        df_prev = fetch_data(prev_sheet)
        if not df_prev.empty:
            df_prev = df_prev.fillna("")
            col_res = 'Tổng CA' if 'Tổng CA' in df_prev.columns else ('Quỹ CA Tổng' if 'Quỹ CA Tổng' in df_prev.columns else None)
            if col_res:
                df_prev[col_res] = pd.to_numeric(df_prev[col_res], errors='coerce').fillna(0.0)
                ton_cu_dict = dict(zip(df_prev['Họ và Tên'], df_prev[col_res]))
        ...
        init_data = {'STT': range(1, len(current_names) + 1), 'Họ và Tên': current_names, 
                     ...
                     'Tồn cũ': [float(ton_cu_dict.get(n, 0.0)) for n in current_names], 'Tổng CA': 0.0}
...
with t1:
    ...
    # Sử dụng Fragment để cô lập phần cập nhật dữ liệu, tránh load lại toàn trang
    @st.fragment
    def render_data_editor():
        ...
        with st.expander("🛠️ CÔNG CỤ CẬP NHẬT NHANH"):
            ...
            f_staff = c1.multiselect("Nhân sự:", active_names, key="ms_staff")
            f_date = c2.date_input("Thời gian:", value=(date(curr_year, curr_month, 1), date(curr_year, curr_month, num_days_curr)), key="di_date")
            r2_1, r2_2, r2_3, r2_4 = st.columns(4)
            f_status = r2_1.selectbox("Trạng thái:", ["Xóa trắng", "Đi Biển", "CA", "WS", "NP", "Ốm"], key="sb_status")
            f_val = r2_2.selectbox("Giàn:", st.session_state.GIANS, key="sb_rig") if f_status == "Đi Biển" else f_status
            f_co = r2_3.selectbox("Công ty:", ["Không đổi"] + COMPANIES, key="sb_co")
            f_ti = r2_4.selectbox("Chức danh:", ["Không đổi"] + TITLES, key="sb_title")
        ...
        # Bọc bảng trong một container cố định
        table_placeholder = st.container()
        with table_placeholder:
            edited_df = st.data_editor(
                st.session_state.db[all_cols],
                use_container_width=True, height=650, hide_index=True, key="main_editor",
                column_config={
            ...
            # Kiểm tra thay đổi để update session_state ngay lập tức
...
# --- 7. TAB BIỂU ĐỒ (GIỮ NGUYÊN LOGIC) ---
with t2:
    ...
    sel_name = st.selectbox("🔍 Chọn nhân sự để xem biểu đồ:", names_for_chart)
    if sel_name:
        results = []
...
# --- 8. SIDEBAR QUẢN LÝ GIÀN (GIỮ NGUYÊN) ---
with st.sidebar:
