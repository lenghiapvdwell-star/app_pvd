...
try:
    ...
    with tab1:
        ...
        with st.form("input_form", clear_on_submit=True):
            c1, c2 = st.columns(2)
            msnv = c1.text_input("Mã số nhân viên *")
        ...
        if submit:
            if msnv and ho_ten:
                ...
                st.success("✅ Đã đồng bộ dữ liệu lên không gian mạng!")
                st.cache_data.clear()
                st.rerun()
            else:
                st.error("⚠️ Thiếu thông tin bắt buộc!")
    ...
    with tab3:
        ...
        st.download_button("📥 TẢI EXCEL (.CSV)", data=csv, file_name='PVD_Data.csv')

except Exception as e:
    st.error("❌ Hàng tiêu đề trên Google Sheet chưa đúng.")
    st.info("Bạn hãy mở Google Sheet ra và điền: MSNV, Họ Tên, Đơn vị, Chức danh, Ngày vào làm, Ghi chú vào hàng đầu tiên nhé!")
