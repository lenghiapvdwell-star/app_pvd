...
# 2. MÃ KHÓA BẢO MẬT (Đã gia cố để chống lỗi PEM)
# Chúng ta dùng r""" để Python giữ nguyên mọi ký tự bên trong
...
try:
    ...
    # Bộ lọc tìm kiếm
    search = st.text_input("🔍 Tìm nhanh nhân viên:")
    if search:
    ...
    st.error(f"❌ LỖI: {e}")
    st.info("Lưu ý quan trọng: Hãy kiểm tra tên Tab trong Google Sheet phải là 'PVD_Data'.")
